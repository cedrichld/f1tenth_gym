...
**Observation-space bounds are physical and finite** (were a blanket `±1e30`, which fails `check_env` normalisation and any bounded-space RL). `_physical_bounds()` ([full.py](f1tenth_gym/envs/observation/full.py)) derives them from the vehicle params and track: velocity from `v_min/v_max`, steering from `s_min/s_max`, pose from the centerline bbox + 5 m margin, angles ±π, yaw-rate `1.5·v_max·tan(s_max)/wheelbase`, frenet `s` from the frame length. The steering/velocity bounds are **padded by one integrator step of actuator overshoot** (`sv_max·integrator_dt`, `a_max·integrator_dt`) — the constraints zero the actuator *rate* only after the angle/speed crosses its limit, so a single hard-steer/hard-accel step lands just past it. `linear_vel_magnitude` is a **true magnitude** `hypot(vx,vy)=|speed|` (not the signed `std_state[3]`), so it never dips below its `0` low when reversing. So `env.observation_space` is a finite `Box` real rollouts stay inside — compose `FlattenObservation` for a normalisable flat space.
...
**Rendering is decoupled from stepping by `RenderClock`** ([f110_env.py](f1tenth_gym/envs/f110_env.py) — class above `F110Env`), driven from `F110Env.render()`. Two independent clocks: a wall-clock **display cap** (human modes redraw at most `render_fps`/wall-second, so stepping faster than real time never forces more frames) and a sim-time **frame accumulator** (rgb_array grabs a distinct frame every `1/render_fps` sim-seconds; the cached frame is returned in between so `RecordVideo` yields smooth video). Human-mode **pacing** holds `sim/wall == real_time_factor` via an absolute anchor (`inf` = free-run). Config: `EnvConfig.render_config` is the **single** rendering config ([`RenderConfig`](f1tenth_gym/envs/env_config.py); the old `RenderSpec` was folded in and retired). Pacing: `render_fps=60`, `real_time_factor=1.0`. Visual: `window_size=800` (also the rgb_array/video resolution), `focus_on="agent_0"` (camera-follow; `None`=map view), `vehicle_palette`, `show_wheels`, `render_map_img`, `car_thickness`, `bigger_car_when_map_centered`, `show_lap_info`. Runtime toggle: `env.unwrapped.set_real_time_factor(x)`; read-only `env.unwrapped.{real_time_factor, render_fps, frame_is_new}`. `human_fast`→rtf 10, `unlimited`→rtf ∞ (legacy sugar). `metadata["render_fps"]` stays `round(1/timestep)` (RecordVideo container fps for real-time playback) — deliberately *not* `render_config.render_fps`.

**Orientation is north-up**: world +x → screen right, +y → up (matches `plt.plot(xs, ys)` of the raceline), set by the camera `_TOP_DOWN_AZIMUTH = -90` in [rendering_pyqtgl.py](f1tenth_gym/envs/rendering/rendering_pyqtgl.py) (azimuth 0 rendered it rotated 90° CW — the `125-rendering-is-flipped-ud` complaint). Only the **2D flat car** exists — the 3D `MeshRenderer`/`example_mesh_cyber.obj`/`car_model` were removed (needed `trimesh`, was config-unreachable). Colliding cars draw red and **revert** to their colour when clear ([pyqtgl_objects.py](f1tenth_gym/envs/rendering/pyqtgl_objects.py) `CarRenderer.render` re-pushes mesh colour only when it changes).
...
- **Warm `step()` ≈ 0.18 ms (≈ 55× real-time at dt=0.01).** Breakdown: LiDAR sphere-trace dominates (njit, scales with `num_beams` — drop beams if you don't need them), then `cartesian_to_frenet` (pure-Python scipy spline eval; disable via `compute_frenet_frame=False` + a non-Frenet `loop_counter` if unused), then RK4. Scales ~linearly, ~0.2 ms/agent. The Frenet spline eval was optimized (~2.9× on its inner loop): `CubicSplineND` reads its cached `spline_x` rather than the scipy `.x` property, and `calc_position_and_yaw` fuses the 4 same-`(s,segment)` polynomial evals into one — **if you touch `cubic_spline.py`, keep those or the `asarray`-per-call cost returns.**
...
| Issue | Evidence |
|---|---|
| ~~A collision ZEROES THE YAW~~ **(FIXED)** — collisions now go through `F110Simulator._halt_on_collision`, which zeros velocity (`[3]`) and rates (`[5:]`) in **both** `state` and `standard_state` while preserving yaw (`[4]`) | [simulator.py](f1tenth_gym/envs/simulator.py) `_halt_on_collision` |
| ~~After a collision `state`/`std_state` DISAGREE~~ **(FIXED)** — `_halt_on_collision` halts both buffers together; `poses` (x,y,yaw) is preserved and stays consistent | same |
| ~~`lidar_config.enabled=False` CRASHES `step()`~~ **(FIXED)** — `_update_agent_collisions` now handles a disabled LiDAR (LIDAR_SCAN skips; BOUNDING_BOX computes vertices via `_compute_all_vertices`). Also added **`CollisionCheckMode.NONE`** to disable collision detection entirely (the answer to upstream #124 "turn off collisions") | [simulator.py](f1tenth_gym/envs/simulator.py) `_update_agent_collisions`, [collision_models.py:38](f1tenth_gym/envs/collision_models.py#L38) |
...
| ~~`obs[...]['scan']` is a VIEW of the live buffer~~ **(FIXED)** — was `scan.astype(np.float32, copy=False)` (aliased the live buffer, corrupting stored scans / RL replay buffers); now `.astype(np.float32)` like the other fields, so it copies | [full.py:125](f1tenth_gym/envs/observation/full.py#L125) |
| ~~`BOUNDING_BOX` REBINDS `state.collisions` every step~~ **(FIXED)** — now `self.state.collisions[:] = np.maximum(...)` writes in place, so a captured `sim.state.collisions` handle stays valid (honouring the SoA in-place invariant) | [simulator.py](f1tenth_gym/envs/simulator.py) `_update_agent_collisions` BOUNDING_BOX branch |
...
| ~~`Track.s_guess` is a SINGLE scalar shared by every agent~~ **(FIXED)** — `simulator.step` now passes each agent's own previous `s` (`state.frenet[i, 0]`) as the search guess, so agents no longer window around each other. `Track.s_guess` remains as a fallback for external single-agent callers | [simulator.py:265-278](f1tenth_gym/envs/simulator.py#L265) |
| ~~FRENET_BASED never re-seeds `agents_prev_s` from the spawn s~~ **(FIXED)** — `reset()` now seeds `agents_prev_s` from `sim.state.frenet[:, 0]` after `sim.reset` (mirroring the WINDING_ANGLE reseed), so laps are measured from spawn, not the spline s=0 datum | [f110_env.py](f1tenth_gym/envs/f110_env.py) reset, after `sim.reset` |
...
| ~~`info` ALIASES the live lap arrays~~ **(FIXED)** — `info` now stores `.copy()` of `lap_times`/`lap_counts`, so a stored info dict no longer mutates retroactively | [f110_env.py](f1tenth_gym/envs/f110_env.py) step/reset info dict |
| ~~`reset(seed=...)` does NOT reseed LiDAR noise~~ **(FIXED)** — `reset()` now derives a noise seed from `self.np_random` (which gymnasium seeds from `reset(seed=...)`) and passes it to `sim.reset(noise_seed=...)`; the noise stream is reproducible per seed and varies across no-seed episodes | [f110_env.py](f1tenth_gym/envs/f110_env.py) reset, [simulator.py](f1tenth_gym/envs/simulator.py) `reset` |
| ~~`truncated` is hardcoded False~~ **(FIXED)** — `TerminationConfig.max_episode_steps` now drives `truncated` (a config-level TimeLimit); `None` = no limit (episodes can still run forever with `max_laps=None`) | [env_config.py](f1tenth_gym/envs/env_config.py) `TerminationConfig`, [f110_env.py](f1tenth_gym/envs/f110_env.py) step |
| ~~Termination watches ONLY the ego~~ **(CONFIGURABLE)** — `TerminationConfig(terminate_on_collision, collision_agents='ego'\|'any')`; default is ego-only (unchanged). `info["collisions"]` now carries per-agent flags. A crashed opponent still halts in place (velocity zeroed, yaw preserved) | [f110_env.py](f1tenth_gym/envs/f110_env.py) `_check_done` |
| ~~`ObservationConfig.features` is silently ignored~~ **(FIXED)** — `ObservationConfig.__post_init__` now raises if `features` is set with a non-`FEATURES` type, instead of silently using the preset | [env_config.py](f1tenth_gym/envs/env_config.py) `ObservationConfig.__post_init__` |
| ~~`ResetConfig` exposes only `strategy`~~ **(FIXED)** — now also `min_dist`, `max_dist`, `shuffle`, `move_laterally` (all `Optional`, `None`=strategy default), forwarded via `ResetConfig.reset_kwargs()` → `make_reset_fn` | [env_config.py](f1tenth_gym/envs/env_config.py) `ResetConfig` |
...
- **`docs/`** is now a **Sphinx** source for the Read the Docs site (`f1tenth-gym.readthedocs.io`), rebuilt for the `EnvConfig`/gymnasium API. `conf.py` uses `sphinx.ext.autodoc` + `napoleon` (GUI deps `pyqtgraph`/`PyQt6`/`OpenGL` are mocked); pages are `installation`/`quickstart`/`configuration`/`observations`/`actions`/`dynamics`/`tracks`/`rendering`/`rewards_and_rl`/`reproducibility`/`examples` + `api/index` (autodoc over the live modules). Build locally: `env -u PYTHONPATH .venv/bin/python -m sphinx -b html docs docs/_build` (needs `sphinx`+`sphinx-rtd-theme`, which `uv sync` does **not** install). RTD build config is the repo-root `.readthedocs.yaml`. The old Doxygen/Breathe tree (`docs/html/`, `docs/xml/`, `Doxyfile`) that documented the dead `f110_gym` package was **removed**. Note: RTD "stable" builds from the legacy line until the project is pointed at `dev-humble`.
